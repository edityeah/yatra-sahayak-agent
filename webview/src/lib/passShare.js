...
// gets shared, so opening/scanning it lands on the pass.
export function passUrl(yatraId) {
  return `${window.location.origin}/yatri/pass?id=${yatraId}`;
}
...
export function whatsappUrl(yatraId, text) {
  return `https://wa.me/?text=${encodeURIComponent(`${text}: ${passUrl(yatraId)}`)}`;
}
