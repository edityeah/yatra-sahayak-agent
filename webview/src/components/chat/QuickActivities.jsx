...
export default function QuickActivities({ activities, onPick, onSeeAll, max = 4 }) {
  const { language } = useLang();
  const chips = activities.slice(0, max);
  return (
        ...
        <div className="flex items-center gap-2 overflow-x-auto -mx-1 px-1 pb-0.5">
          {chips.map((a) => (
            <button
              key={a.id}
              onClick={() => onPick(a)}
              className="flex-shrink-0 inline-flex items-center gap-2 h-10 px-4 rounded-full bg-white border border-bdr hover:border-primary shadow-card text-[13px] font-bold text-ink transition"
            >
              <span className="text-[15px]">{a.icon}</span>
              {t(a.label, language)}
            </button>
          ))}
          {onSeeAll && activities.length > chips.length && (
            <button
              onClick={onSeeAll}
              className="flex-shrink-0 inline-flex items-center h-10 px-3 rounded-full bg-primary-50 text-primary text-[12px] font-bold hover:bg-primary-100"
            >
              See all
            </button>
          )}
